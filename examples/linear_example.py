...
# Data normalization
x_linear_train_norm = (x_linear_train - np.mean(x_linear_train, axis=0))/(np.std(x_linear_train, axis=0))
x_linear_test_norm = (x_linear_test - np.mean(x_linear_test, axis=0))/(np.std(x_linear_test, axis=0))
...
x_min, x_max = x_linear_train_norm[:, 0].min() - 0.5, x_linear_train_norm[:, 0].max() + 0.5
y_min, y_max = x_linear_train_norm[:, 1].min() - 0.5, x_linear_train_norm[:, 1].max() + 0.5
...
ax.scatter(x_linear_train_norm[:, 0], x_linear_train_norm[:, 1], c=y_linear_train, cmap=cm_bright, edgecolors="k")
